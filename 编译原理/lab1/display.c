...
{//对抽象语法树的先根遍历
  ...
  if (T)
    ...
    case EXT_DEC_LIST:  display(T->ptr[0],indent);     //依次显示外部变量名，
                        display(T->ptr[1],indent);     //后续还有不少相同的形式的代码，仅显示语法树此处理代码可以和类似代码合并
                        break;
	...
	case PARAM_LIST:    display(T->ptr[0],indent);     //依次显示全部参数类型和名称，
                        display(T->ptr[1],indent);     //仅显示语法树此处理代码可以和类似代码合并
                        break;
	case PARAM_DEC:     printf("%*c类型：%s, 参数名：%s\n",indent,' ',T->ptr[0]->type,T->ptr[1]->type_id);
                        break;
    case ARRAY_DEF:
                        printf("%*c一维数组定义： \n",indent,' ');
                        display(T->ptr[0],indent+3);
                        printf("%*cSIZE: %d\n",indent+3,' ',T->array_size[0]);
                        break;
	case ARRAY_ITEM:
                        printf("%*c一维数组变量： \n",indent,' ');
                        display(T->ptr[0],indent+3);
	...
	case EXP_STMT:      printf("%*c表达式语句：\n",indent,' ');
                        display(T->ptr[0],indent+3);
                        break;
	case RETURN:        printf("%*creturn语句：\n",indent,' ');
                        display(T->ptr[0],indent+3);
                        break;
	case COMP_STM:      printf("%*c程序块：\n",indent,' ');
                        printf("%*c程序块的局部变量定义：\n",indent+3,' ');
    ...
    case DEC_LIST:      printf("%*cVAR_NAME：\n",indent,' ');
                        while (T->kind==DEC_LIST) {
                            if (T->ptr[0]->kind==ID)
                                printf("%*c %s\n",indent+3,' ',T->ptr[0]->type_id);
                            else if (T->ptr[0]->kind==ASSIGNOP)
                                {
                                printf("%*c %s ASSIGNOP\n ",indent+3,' ',T->ptr[0]->ptr[0]->type_id);
                                display(T->ptr[1],indent+6);        //显示初始化表达式
                                }
                            T=T->ptr[1];
                            }
                        if (T->kind==ID)
                                printf("%*c %s\n",indent+3,' ',T->type_id);
                        else if (T->kind==ASSIGNOP)
                                {
                                printf("%*c %s ASSIGNOP\n ",indent+3,' ',T->ptr[0]->type_id);
                                display(T->ptr[1],indent+6);        //显示初始化表达式
                                }
                        break;
	case ID:	        printf("%*c标识符： %s\n",indent,' ',T->type_id);
                        break;
    ...
    case CHAR:          printf("%*cCHAR：%s\n",indent,' ',T->type_char);
                        break;
	case ASSIGNOP:      printf("%*cASSIGNOP： %s\n",indent,' ',T->type_id);
                        display(T->ptr[0],indent+3);
    ...
    case FUNC_CALL: printf("%*c函数调用：\n",indent,' ');
                    ...
                    break;
	case ARGS:      i=1;
                    while (T->kind==ARGS) {  //ARGS表示实际参数表达式序列结点，其第一棵子树为其一个实际参数表达式，第二棵子树为剩下的
