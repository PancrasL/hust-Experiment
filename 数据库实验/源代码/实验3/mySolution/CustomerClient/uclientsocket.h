...
#include <QObject>
class QTcpSocket;
class UClientSocket : public QObject
{
    Q_OBJECT
public:
    UClientSocket();
...
private slots:
    ...
    void handleTryGetUserInfo(const QString &uid);
    void handleRentCar(const QString &msg);
    void handleConfirmRentCar(const QString &msg);
    void handleGetHistoryOrder(const QString &msg);
signals:
    ...
    void getUserInfoReply(const QVector<QString> &info);
    void getRentCarReply(const QVector<QString> &info);
    void confirmRentCarReply(const QString &msg);
    void getHistoryOrderReply(const QVector<QString> &info);
};
